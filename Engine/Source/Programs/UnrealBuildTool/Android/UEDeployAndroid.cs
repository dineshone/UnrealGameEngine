...
*/
			...
			// SOLUTION: I am just going to put the entire method into a if... elseif... statement
			//
			// look up an ANT_HOME env var
			string AntHome = Environment.GetEnvironmentVariable ("ANT_HOME");
			string PluginDir = Environment.ExpandEnvironmentVariables ("%ANDROID_HOME%/../eclipse/plugins");

			Log.TraceInformation ("\n====d0d0246==== " + AntHome + Directory.Exists (AntHome) + string.IsNullOrEmpty (AntHome) + PluginDir + Directory.Exists (PluginDir));

			if (!string.IsNullOrEmpty (AntHome) && Directory.Exists (AntHome)) {
				string AntPath = AntHome + "/bin/ant" + (Utils.IsRunningOnMono ? "" : ".bat");
				//string AntPath = AntHome +  (Utils.IsRunningOnMono ? "" : ".bat");
				// use it if found
				Log.TraceInformation ("\n====d0d0256==== " + AntPath);
				return AntPath;
//				if (File.Exists(AntPath))
...
//				}
		...
		private static string GetNDKArch (string UE4Arch)
		{
			...
			default:
				throw new BuildException ("Unknown UE4 architecture {0}", UE4Arch);
			}
		}
...
//				default: throw new BuildException("Unknown NDK architecture '{0}'", NDKArch);
		...
		private static void RunCommandLineProgramAndThrowOnError (string WorkingDirectory, string Command, string Params, string OverrideDesc = null, bool bUseShellExecute = false)
		{
			...
			if (Proc.ExitCode != 0) {
				...
				*/
				...
				// d0d0 03/29/2016 I made the following changes to better trace the error that was occurring.
				// The original managed exception wasn't giving out a useful message.

				Log.TraceError ("\n d0d0724 Error running: " + Command + " " + Params + " from directory " + WorkingDirectory);
				throw new BuildException ("{0} failed with args {1}", Command, Params, " WorkingDirectory is: ", WorkingDirectory);
		...
		private void UpdateProjectProperties (AndroidToolChain ToolChain, string UE4BuildPath, string ProjectName)
		{
			...
			// d0d0 05/13/2016 from some reason, the android project is not built by the unreal editor.
			// I get the message no Build.xml in the project.
			// When i build the project, the lighting and the swarm does not execute and this stops the build.
			// This happens only in the editor thats beene xecuted from the source code.
			// The editor form the executable works just fine
			// I dont know why this happens and as a short term solution, I am going to build the project myself
			// I am going to initiate an ant build ...

			Log.TraceInformation ("\n====d0d0 checking for build.xml ====================================================", DateTime.Now.ToString ());

			if (!File.Exists ("build.xml")) {

				Log.TraceInformation ("\n====d0d0 checking for build.xml in Update Project Properties method====================================================", DateTime.Now.ToString ());
				Log.TraceInformation ("\n====" + UE4BuildPath, AndroidCommandPath, "--silent update project --path " + UE4BuildPath + " --target " + GetSdkApiLevel (ToolChain), DateTime.Now.ToString ());

				//	RunCommandLineProgramAndThrowOnError (UE4BuildPath, AndroidCommandPath, " update project --path " + UE4BuildPath + " --target " + GetSdkApiLevel (ToolChain), "");

			}
			foreach (string Lib in LibsToBeAdded) {
				string LocalUpdateCommandLine = UpdateCommandLine + " --library " + Lib;
				Log.TraceInformation ("\n====d0d0 checking LocalUpdateCommandLine=====================================");
		...
		private string GenerateManifest (string ProjectName, bool bIsForDistribution, bool bPackageDataInsideApk, string GameBuildFilesPath, bool bHasOBBFiles, bool bDisableVerifyOBBOnStartUp, string UE4Arch, string GPUArch)
		{
			...
			// d0d0 05/20/2016 The activities are stored under different folders under src
			// so in the manifest, when I mention package name = com.epicgames.ue4, it looks for activities only 
			// under the folder src/com/epicgames/ue4/ but my activities are in a different folder. 
			// So instead of mentioning each and every folder, I am changing the package name to 'com'...
			// lets see if this works.
			//Text.AppendLine (string.Format ("          package=\"{0}\"", PackageName));
			Text.AppendLine (string.Format ("          package=\"{0}\"", "com"));
			...
			if (bShowLaunchImage) {
				...
				*/
				...
				// d0d0 03/21/2016 4PM ...
				//	This is where I add my Activity "com.hazardnetworking.unrealandroidtest1.LoginActivity"
				// To start with I will just add my default activity and once I am able to get it to work,
				// I will add the GameActivity back to the manifest.
				// I am commenting out the default activity
				//Text.AppendLine("\t\t<activity android:name=\"com.epicgames.ue4.GameActivity\"");
				//
				// d0d0 )5/02/2016 I changed the Activity name and the Android API version to 21
				// d0d0 05/05/2016 I changed the theme from UE4SplashTheme to AppTheme
				//
				Text.AppendLine ("\t\t<activity android:name=\"com.hazardnetworking.unrealandroidtest2.MainActivity\"");
				Text.AppendLine ("\t\t          android:label=\"@string/app_name\"");
				Text.AppendLine ("\t\t          android:theme=\"@style/AppTheme\"");
				Text.AppendLine ("\t\t          android:configChanges=\"screenSize|orientation|keyboardHidden|keyboard\"");
			} else {
				// d0d0 I am commenting out this
				//Text.AppendLine("\t\t<activity android:name=\"com.epicgames.ue4.GameActivity\"");
				Text.AppendLine ("\t\t<activity android:name=\"com.hazardnetworking.unrealandroidtest2.MainActivity\"");
				Text.AppendLine ("\t\t          android:label=\"@string/app_name\"");
			...
			//
			// d0d0 04/20/2016
			// I am adding the GameActivity as a second activity,
			// this is where the unreal graphics are added to the android application
			// I am also adding the meta-data for lib_name as UE4. I missed adding this part and it crashes
			// 
			//
			...
			if (File.Exists (RequirementsOverrideFile)) {
				...
				*/
				...
				//d0d0 03/21/2016 4:05PM
				// I am adding more permissions READ_PROFILE, READ_CONTACTS
				Text.AppendLine ("\t<uses-permission android:name=\"android.permission.READ_PROFILE\"/>");
				Text.AppendLine ("\t<uses-permission android:name=\"android.permission.READ_CONTACTS\"/>");
		...
		private void MakeApk (AndroidToolChain ToolChain, string ProjectName, string ProjectDirectory, string OutputPath, string EngineDirectory, bool bForDistribution, string CookFlavor, bool bMakeSeparateApks, bool bIncrementalPackage, bool bDisallowPackagingDataInApk)
		{
			...
			var templates = from template in Directory.EnumerateFiles (JavaTemplateSourceDir, "*.template")
				...
				SourceFile = template,
				DestinationFile = GetUE4TemplateJavaDestination (TemplateDestinationBase, RealName)
			};
			...
			if (bBuildSettingsMatch) {
				...
				JavaFiles.AddRange (from t in templates
				                    select t.SourceFile);
				JavaFiles.AddRange (from t in templates
				                    select t.DestinationFile);
			...
			foreach (var build in BuildList) {
				...
				Log.TraceInformation ("\n========d0d0 build in buildList ========" + build + DateTime.Now.ToString ());
				...
				if (HasNDKPath) {
					...
					Log.TraceInformation ("\n====d0d0 1833  no ndk path ================================================" + DateTime.Now.ToString () + HasNDKPath);
				...
				if (HasNDKPath) {
					...
					if (!File.Exists (LibSOName) || Diff.TotalSeconds < -1 || Diff.TotalSeconds > 1) {
						...
						Log.TraceInformation ("\n====d0d0 1862 running ndk-build ================================================" + DateTime.Now.ToString () + UE4BuildPath + NDKBuildPath + CommandLine);
				...
				*/
					...
					// build build.xml if for some reason, it has not beeen created.
					// d0d0 05/13/2016 from some reason, the android project is not built by the unreal editor.
					// I get the message no Build.xml in the project.
					// When i build the project, the lighting and the swarm does not execute and this stops the build.
					// This happens only in the editor thats beene xecuted from the source code.
					...
					if (!File.Exists ("build.xml")) {
						...
						Log.TraceInformation ("\n d0d0 1919 ====d0d0 checking for build.xml ====================================================" + DateTime.Now.ToString ());
						Log.TraceInformation ("\n====" + "UE4BuildPath:" + UE4BuildPath + " AndroidCommandPath: " + AndroidCommandPath + "update project --path " + UE4BuildPath + " --target " + GetSdkApiLevel (ToolChain), DateTime.Now.ToString ());

						RunCommandLineProgramAndThrowOnError (UE4BuildPath, AndroidCommandPath, " update project --path " + "'" + UE4BuildPath + "'" + " --target " + GetSdkApiLevel (ToolChain), "");

					}

					// d0d0 03/31/2016 I want the ant options to be verbose. I want to see more messages that help me debug the build.
					...
					//I am changing -quiet option to -verbose

					//d0d01911 05/17/2016 the RunCommandLineProgram was throwing an error. I was supplying AntBuildType instead of 
					// AntOutputSuffix. 

					Log.TraceInformation ("\n====d0d02049==== " + UE4BuildPath + " " + GetAntPath (), " -verbose " + AntOutputSuffix + " Making .apk with Ant... (note: it's safe to ignore javac obsolete warnings)");

					RunCommandLineProgramAndThrowOnError (UE4BuildPath, GetAntPath (), " -verbose " + AntOutputSuffix, " Making .apk with Ant... (note: it's safe to ignore javac obsolete warnings)");
				} else {
					RunCommandLineProgramAndThrowOnError (UE4BuildPath, "cmd.exe", "/c \"" + GetAntPath () + "\" " + AntOutputSuffix, "Making .apk with Ant... (note: it's safe to ignore javac obsolete warnings)");
				}
				...
				try {
					...
					Log.TraceInformation ("\n d0d01966 " + e + Directory.Exists (DestApkName) + UE4BuildPath + "/bin/" + ProjectName + AntOutputSuffix + ".apk");
				}
		...
		public override bool PrepTargetForDeployment (UEBuildTarget InTarget)
		{
			...
			Log.TraceInformation ("d0d0 2018 MakeApk called in PrepareTargetForDeployment method");
		...
		public override bool PrepForUATPackageOrDeploy (FileReference ProjectFile, string ProjectName, string ProjectDirectory, string ExecutablePath, string EngineDirectory, bool bForDistribution, string CookFlavor, bool bIsDataDeploy)
		{
			...
			Log.TraceInformation ("d0d0 2018 MakeApk called in PrepForUATPackageOrDeploy method");
		...
		private void UpdateGameActivity (string UE4Arch, string NDKArch, string EngineDir, string UE4BuildPath)
		{
			...
			Dictionary<string, string> Replacements = new Dictionary<string, string> {
				...
				{ "//$${gameActivityClassAdditions}$$", APL.ProcessPluginNode (NDKArch, "gameActivityClassAdditions", "") }, {
					...
					APL.ProcessPluginNode (NDKArch, "gameActivityReadMetadataAdditions", "")
				},
				...
				{ "//$${gameActivityOnResumeAdditions}$$", APL.ProcessPluginNode (NDKArch, "gameActivityOnResumeAdditions", "") }, {
					...
					APL.ProcessPluginNode (NDKArch, "gameActivityOnActivityResultAdditions", "")
				},
